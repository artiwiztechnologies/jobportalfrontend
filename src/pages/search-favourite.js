            ...
            <div className="pt-9 pl-sm-9 pl-5 pr-sm-9 pr-5 pb-8 border-bottom border-width-1 border-default-color light-mode-texts">
              ...
              <div className="row pt-9">
                <div className="col-12">
                  ...
                  <div className="card-btn-group">
                    <button
                      className="btn btn-blue text-uppercase btn-medium rounded-3 w-180 mr-4 mb-5"
                      onClick={() => {
...
const SearchFavorites = () => {
  ...
  const [jobsSavedData, setJobsSavedData] = useState([]);
  const [actualJobData, setActualJobData] = useState([]);
  let testData = [];
  const actualjData1 = [];
  ...
  useEffect(() => {
    if (isAuthenticated() && isAuthenticated().active==true) {
      ...
      getFav(isAuthenticated().access_token).then((d1) => {
        console.log("fav jobs data:");
        console.log(d1);
  ...
  // if (jobsSavedData?.length != null) {
  //   // console.log(jobsSavedData);
  //   for (let i = 0; i < jobsSavedData.length; i++) {
  //     // console.log(jobsSavedData[i].id);
  //     getJobFromId(jobsSavedData[i].id, isAuthenticated().access_token).then(
  //       (data) => {
  //         console.log(data);

  //         console.log("favourites data after adding:");
  //       }
  //     );
  //   }
  // }

  const FavsJobCard = ({ id, handleshow }) => {
    const [job, setJob] = useState();
    getJobFromId(id, isAuthenticated().access_token).then((data) => {
      // console.log(data);
      setJob(data);
    ...
    if (job != undefined) {
      return (
        <div style={{ cursor: "pointer" }} className="col-12 col-lg-6">
          <div
            onClick={() => {
              handleShow(job);
            }}
                ...
                <div className="pt-12 ml-lg-0 ml-md-15">
                  ...
                  <div className="pt-6">
                    <div className="row justify-content-center">
                      ...
                      {jobsSavedData?.map((job) => (
                        <FavsJobCard handleshow={handleShow} id={job.id} />
                      ))}
                      <ModalViewJobDetails
                        show={show}
